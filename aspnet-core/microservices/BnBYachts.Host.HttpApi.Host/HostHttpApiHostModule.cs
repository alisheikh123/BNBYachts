...
using BnBYachts.Host.MultiTenancy;
...
namespace BnBYachts.Host
{
    ...
    public class HostHttpApiHostModule : AbpModule
    {
        ...
        {
            Configure<AbpDistributedCacheOptions>(options => { options.KeyPrefix = "Host:"; });
        }
                ...
                Configure<AbpVirtualFileSystemOptions>(options =>
                {
                    options.FileSets.ReplaceEmbeddedByPhysical<HostDomainSharedModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $"..{Path.DirectorySeparatorChar}BnBYachts.Host.Domain.Shared"));
                    options.FileSets.ReplaceEmbeddedByPhysical<HostDomainModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $"..{Path.DirectorySeparatorChar}BnBYachts.Host.Domain"));
                    options.FileSets.ReplaceEmbeddedByPhysical<HostApplicationContractsModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $"..{Path.DirectorySeparatorChar}BnBYachts.Host.Application.Contracts"));
                    options.FileSets.ReplaceEmbeddedByPhysical<HostApplicationModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $"..{Path.DirectorySeparatorChar}BnBYachts.Host.Application"));
                });
            }
        }
            ...
            IWebHostEnvironment hostingEnvironment)
        {
            if (!hostingEnvironment.IsDevelopment())
            {
                ...
                context.Services
                    .AddDataProtection()
                    .PersistKeysToStackExchangeRedis(redis, "Host-Protection-Keys");
            }
        }
