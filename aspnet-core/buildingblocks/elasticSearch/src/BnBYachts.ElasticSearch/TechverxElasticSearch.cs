...
{
    public class TechverxElasticSearch : ITechverxElasticSearch
    {
        public IElasticClient EsClient { get; set; }
        public TechverxElasticSearch(IOptions<ElasticSearchConfiguration> elasticSearchConfiguration)
        {
            ...
            params string[] highField) where T : EntityDto<TKey>
        {
            ...
            if (disableHigh)
            {
                preTags = string.Empty;
                postTags = string.Empty;
            }

            highlight.PreTags(preTags).PostTags(postTags);

            var isHigh = highField is { Length: > 0 };

            var hfs = new List<Func<HighlightFieldDescriptor<T>, IHighlightField>>();
            query.Skip(skip).Take(size);
            if (isHigh)
            {
                hfs.AddRange(highField.Select(s => (Func<HighlightFieldDescriptor<T>, IHighlightField>)(f => f.Field(s))));
            }

            highlight.Fields(hfs.ToArray());
            query.Highlight(h => highlight);
            if (includeFields != null) query.Source(ss => ss.Includes(ff => ff.Fields(includeFields.ToArray())));
