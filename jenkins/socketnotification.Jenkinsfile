...
echo Paymnet_URL: ${IMAGE_URL}:${IMAGE_TAG}  >> build_info.md
...
'''
    ...
    stage('Phase-1') {
      parallel {
          ...
          stage('socket api and notifcation') {
            stages {
              stage('Build') {
                steps {
                  script {
                    sh "docker build -t ${IMAGE_NAME}:${IMAGE_TAG} \
                      -f aspnet-core/Socket.Dockerfile ./aspnet-core "
                  }
                }
              }
              stage('Publish') {
 ...
 stage('Cleanup') {
  ...
  environment {
    ...
    IMAGE_NAME = 'bnb-socketnotification'
